...
**Rozstrzygnięte przez [ADR-63](../adr/ADR-63-embedding-model-placement.md).** Worker dostaje `[[bin]] smartfs-worker` i żyje w **osobnym procesie**, nadzorowanym przez `smartfsd` jako dziecko z `PDEATHSIG`. Osobny proces, bo upadek biblioteki ML (1,2 GB wag, natywny kod C++, docelowo sterownik GPU) nie może odmontować systemu plików; dziecko demona, a nie unit systemd, bo „trzeba pamiętać włączyć” to dokładnie ten tryb awarii, który wystąpił w tym projekcie trzy razy.
...
- **Silnik embeddingów był atrapą — rozstrzygnięte przez [ADR-63](../adr/ADR-63-embedding-model-placement.md).** Przy pisaniu ADR-a o miejscu workera wyszło coś gorszego niż brak punktu wejścia: `CpuEmbeddingEngine` nie uruchamiał żadnego modelu. Liczył SHA-256 z tekstu i rozwijał go w wektor, z doc-commentem mówiącym „ONNX model compatibility" i `Cargo.toml` bez jakiejkolwiek zależności inferencyjnej. Gdyby worker był podłączony, tabele embeddingów **zapełniłyby się**, etap 6 przeszedłby na zielono, a `search_semantic` zwracałoby losowe pliki — brak workera był jedyną rzeczą, która to ukrywała przed użytkownikiem zamiast przed testem. Widać tu, że etap 6 w obecnym kształcie nadal by tego nie złapał: sprawdza, że wynik jest niepusty, nie że jest sensowny. Asercja jakości (funkcja pasująca do opisu wyżej niż losowa inna z tego samego przebiegu) jest w §Konsekwencje ADR-63 jako zadanie przy implementacji.
- **Wymiar AST był niespójny od początku.** `embed_version` wstawiał wektor o 1536 wymiarach do `ast_embeddings_1536`, podpisując go `model_id` modelu 1024-wymiarowego. Prawdziwy model nie umie tego wyprodukować — kod mógł działać wyłącznie dzięki atrapie. Migracja 010 (ADR-63 §5) domyka cutover, który ADR-49 przeprowadził na poziomie plików i przeoczył na poziomie AST.

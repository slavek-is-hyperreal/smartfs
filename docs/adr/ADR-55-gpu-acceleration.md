...
3. Jest to spójne z już zaakceptowanym wzorcem w projekcie: `ort` (ONNX Runtime, silnik CPU-baseline z ADR-49) też jest bindingiem Rust do biblioteki C++. FFI do dojrzałego silnika inferencji nie jest nowym rodzajem kompromisu — to kontynuacja tego samego wzorca, tylko dla ścieżki GPU.

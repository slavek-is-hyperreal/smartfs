...
**Status:** Zaimplementowany (2026-09-13), kroki A-F. Punkt 7 Decyzji (nakładka odczytu) dopisany w trakcie implementacji — patrz jego treść; bez niego ADR łamał read-after-write. Wszystkie trzy Otwarte pytania rozstrzygnięte przez właściciela projektu — patrz §Rozstrzygnięcia. Punkt 6 niżej pozostaje **kandydatem** do rozszerzenia listy Root Invariants, nie został dopisany automatycznie.
...
7. **Nakładka odczytu nad kolejką — DOPISANE 2026-09-13, przy kroku F.** Punkty 1-6 nie mówią nic o czytaniu, i to jest luka, nie przeoczenie warte zignorowania: skoro `release()` potwierdza przed commitem, to `inode_registry.current_blob_id` i `size` opisują w tym oknie **poprzednią** wersję. Bez dodatkowego mechanizmu `echo x > f; cat f` zwraca starą treść, a `stat` stary rozmiar — świeży `open()` ma pusty bufor i spada do bazy (`fs.rs`, ścieżka `read`). To jest złamanie POSIX-owego read-after-write, nie kosmetyka, i wywaliłoby etapy 2 i 3 planu testowego błędami, których nie da się zakwalifikować ani jako „FUSE-inherent", ani jako udokumentowane ograniczenie MVP. Dlatego każdy nieodhaczony znacznik publikuje pod swoim inode `(blob_id, size, content_hash)`, a ścieżki `read()` i `getattr()` zaglądają tam **przed** bazą. Wpis znika, gdy wiersz staje się realny, a wycofanie jest kluczowane nazwą znacznika, żeby zakończenie starszego commitu nie skasowało nowszego zapisu. Nakładka jest ograniczona tym samym licznikiem co kolejka (nie rośnie bez końca) i jest odtwarzana ze skanu `pending/queue/` **zanim** demon ogłosi gotowość — znaczniki z poprzedniego uruchomienia opisują zapisy, których wołający już usłyszał, że się udały.
8. **Osobna, rzadsza pętla: scrub sumami kontrolnymi już zatwierdzonych blobów** (bit rot, nie osierocenie) — porównanie zawartości na dysku z zapisanym `content_hash` dla losowej próbki albo pełnego przebiegu przy niskim obciążeniu. To jest mechanizm inny niż punkt 4 (tam wykrywamy brak wiersza w bazie dla istniejącego pliku; tu wykrywamy zepsuty plik dla istniejącego wiersza) i nie może być z nim pomylony w implementacji ani w logach.
...
- Scrub sum kontrolnych (punkt 8 Decyzji) to osobny, nowy komponent w `smartfs-store` — nie rozszerzenie skanu `pending/`.
...
| D | `smartfs-store`: pętla scrub z punktu 8 Decyzji | niezależnie, może iść równolegle z B/C | krok A |
